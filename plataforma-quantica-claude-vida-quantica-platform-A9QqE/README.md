...
Plataforma web (área do aluno) oferecida como **bônus exclusivo** para quem
adquire o curso **Vida Quântica** na Hotmart. Não é uma plataforma de venda —
o aluno só acessa após a compra ser confirmada.
...
### Admin
- Painel com métricas, liberação manual de acesso, gestão de mantras,
  meditações, conteúdos da biblioteca e lives.

### Integração Hotmart
- Webhook que cria conta + libera acesso automaticamente após compra aprovada;
  revoga em reembolso/chargeback.
...
- Admin (se ADMIN_EMAIL/PASSWORD definidos): aquele do `.env`
...
2. Crie um serviço Node apontando para `apps/api`:
   ...
   - **Root Directory**: `/`
3. Variáveis: `DATABASE_URL`, `JWT_SECRET`, `WEB_URL` (URL pública da Vercel),
   `HOTMART_WEBHOOK_TOKEN`, `ADMIN_EMAIL`, `ADMIN_PASSWORD`, `NODE_ENV=production`.
...
Comportamento:
- **Aprovação**: cria usuário (se novo) + ativa `Access`. Senha temporária
  retornada no payload — envie ao aluno (ou implemente fluxo de "esqueci a senha").
